...
export const getCountry=(params)=>dispatch=>{
    ...
    return axios.get("http://localhost:8080/countrie",params).then((r)=>{
...
export const upadatecity=(id,payload)=>dispatch=>{
    dispatch({type:types.UPDATE_COUNTRY_REQUEST})
    return axios.patch(`http://localhost:8080/countrie/${id}`,payload).then((r)=>{
...
export const  deletecity=(id)=>dispatch=>{
    ...
    return axios.delete(`http://localhost:8080/countrie/${id}`).then((r)=>{
