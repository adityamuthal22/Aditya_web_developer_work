...
export const CartProvider = ({ children }) => {
  ...
  const removeItemFromCart = async (productId) => {
    ...
    if(item.id) {
      return axios.delete(`http://localhost:8080/cartItems/${item.id}`).then(() => {
  ...
  const updateItemCount = async(productId, newCount) => {
    if(newCount == 0) {
      ...
      let item = cartItems.find((item) => item.productId == productId) || {};
      if(item.id) {
        return axios.patch(`http://localhost:8080/cartItems/${item.id}`, {
          count: newCount,
        }).then(({data}) => {
  ...
  return <CartContext.Provider value={{cartItemsCount: cartItems.length, cartItems, getCartItemByProductId, addItemToCart, removeItemFromCart, updateItemCount}} >{children}</CartContext.Provider>;
};
